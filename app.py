...
ultima_publicacion_calidad = 0  # Para simular calidad del aire
...
def leer_serial():
    ...
    global contador, led_encendido, ultimo_contador, ultima_publicacion_calidad
    try:
        if arduino.in_waiting > 0:  # Verificar si hay datos disponibles
            ...
            if mensaje:  # Si se recibe un mensaje válido
                ...
                if contador != ultimo_contador:
                    # Publicar en Personas
                    mqtt_client.publish(mqtt_topic_personas, json.dumps({
                        "timestamp": datetime.now().isoformat(),
                        "personas": contador
                    }))
                    print(f"Publicado contador en {mqtt_topic_personas}: {contador}")
                    ultimo_contador = contador
    except Exception as e:
        print(f"Error al leer serial: {e}")

    # Simular datos de calidad del aire cada 5 segundos (para probar el topic)
    tiempo_actual = time.time()
    if tiempo_actual - ultima_publicacion_calidad >= 5.0:
        calidad_simulada = 400 + (contador * 10)  # Valor simulado (puedes ajustar)
        mqtt_client.publish(mqtt_topic_calidad_aire, json.dumps({
            "timestamp": datetime.now().isoformat(),
            "calidad_aire_ppm": calidad_simulada
        }))
        print(f"Publicado calidad del aire simulada en {mqtt_topic_calidad_aire}: {calidad_simulada} ppm")
        ultima_publicacion_calidad = tiempo_actual
...
while True:
    ...
    # Leer datos del Arduino y manejar MQTT
    leer_serial()
